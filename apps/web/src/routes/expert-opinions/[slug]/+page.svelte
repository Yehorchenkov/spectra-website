<script>
	...
	let expertOpinionItem = $derived(data.expertOpinion);

	// Always show a header image: use the article image when available, otherwise a placeholder.
	const headerImageUrl = $derived(expertOpinionItem?.image?.sizes?.card?.url ?? expertOpinionItem?.image?.url ?? EXPERT_OPINIONS_PLACEHOLDER);
	const headerImageAlt = $derived(expertOpinionItem?.image?.alt ?? expertOpinionItem?.title ?? 'Expert Opinion');

	const publishDateLabel = $derived(
		expertOpinionItem?.publishDate ? formatDateLong(expertOpinionItem.publishDate) : null
	);
</script>

<SEO
	title={expertOpinionItem.meta.title}
	description={expertOpinionItem.meta?.description}
	collection="Expert Opinion"
/>

{#if expertOpinionItem}
		...
		<article class="space-y-8">
				...
				<div class="grid gap-6 md:grid-cols-[1fr_360px] md:items-start">
					<div class="space-y-4">
						<h1 class="text-3xl md:text-4xl font-bold tracking-tight text-foreground">
							{expertOpinionItem.title}
						</h1>

						<div class="flex flex-wrap items-center gap-x-8 gap-y-3 text-sm text-muted-foreground">
							{#if expertOpinionItem.publishDate}
								<div class="flex items-center gap-2">
									<CalendarDots class="size-5 flex-shrink-0" />
									<time datetime={expertOpinionItem.publishDate}>{publishDateLabel}</time>
								</div>
							{/if}

							{#if expertOpinionItem.tags?.length > 0}
								<div class="flex items-center gap-2 flex-wrap">
									<Tag class="size-5 shrink-0" />
									{#each expertOpinionItem.tags as t}
			...
			<section class="space-y-4">
				<div class="max-w-none text-foreground leading-relaxed">
					<RichTextRenderer content={expertOpinionItem.content} />
				</div>
			</section>
		</article>
	</div>
{:else}
	<div class="flex justify-center items-center h-64">
		<p class="text-muted-foreground">Expert opinion not found.</p>
	</div>
{/if}
