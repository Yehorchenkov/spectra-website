import { t as attr_class, u as clsx } from "./index.js";
...
function convertContentToHTML(content) {
  const htmlConverters = ({ defaultConverters }) => ({
    ...
    heading: ({ node, childrenConverters }) => {
      ...
      const childrenHTML = node.children?.map((child) => {
        if (child.text) {
          ...
          if (child.format & 1) text2 = `<strong>${text2}</strong>`;
          if (child.format & 2) text2 = `<em>${text2}</em>`;
          return text2;
        }
        return "";
      }).join("");
    ...
    blocks: {
      ...
      imageGallery: ({ node }) => {
        const images = node.fields.images.map((image) => {
          return `<a
                        href="${image.url}"
...
function RichTextRenderer($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { class: className, content } = $$props;
    $$renderer2.push(`<div${attr_class(clsx(twMerge("prose dark:prose-invert max-w-none text-justify prose-a:text-primary prose-a:no-underline prose-img:rounded-lg", className)))}>${html(convertContentToHTML(content))}</div>`);
  });
}
export {
